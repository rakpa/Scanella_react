...
class _DotGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = Brand.accent.withValues(alpha: 0.16);
    const columns = 7;
    const rows = 5;
...
class _ScannerDevicePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // The open lid.
    //
    // This was two dark navy rectangles, which at a glance read as a black
    // slab floating behind the scanner rather than as part of it — the one
    // piece of the intro art that looked unfinished. A real lid seen from the
    // front is a pale pressure pad in a dark frame, and a hinge joins it to
    // the body, so it is drawn that way.
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(w * 0.15, h * 0.06, w * 0.70, h * 0.30),
        Radius.circular(w * 0.045),
      ),
      Paint()..color = const Color(0xFF33456B),
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(w * 0.19, h * 0.10, w * 0.62, h * 0.22),
        Radius.circular(w * 0.03),
      ),
      Paint()..color = const Color(0xFFDDE3EE),
    );
    // A soft shadow inside the pad so it reads as recessed, not as a sticker.
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(w * 0.19, h * 0.10, w * 0.62, h * 0.05),
        Radius.circular(w * 0.03),
      ),
      Paint()..color = const Color(0xFFC3CCDD),
    );
    // Hinges, joining the lid to the body.
    for (final x in [0.32, 0.68]) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(w * x, h * 0.35, w * 0.06, h * 0.13),
    ...
    canvas.drawRRect(
      ...
      Paint()..color = Brand.accent.withValues(alpha: 0.55),
    );
...
class _ScanningPhonePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bracket = Paint()..color = Brand.accent;
    for (final corner in [
...
class _SecureFolderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
      ...
      Paint()
        ..color = Brand.accent.withValues(alpha: 0.28)
        ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 16),
    );
    canvas.drawRRect(folder, Paint()..color = Brand.accent);
    ...
    canvas.drawRRect(lockBody, Paint()..color = Brand.accent);
    canvas.drawArc(
      Rect.fromCenter(
